...
def test_serialization_transformer(model):
    ...
    assert loaded_model.get_model().to_yaml() == model.to_yaml()


def test_serialization_estimator(model):
...
def test_spark_ml_model(spark_context, model, mnist_data):
    batch_size = 64
    nb_classes = 10
    epochs = 1
